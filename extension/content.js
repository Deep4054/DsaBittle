...
(function () {
  ...
  function renderInsights(insights, data) {
    ...
    // Support both new fields (whySolveThis/whereUsed/whyCompaniesAsk)
    // and old fields (whyMatters/useCases) for backwards compatibility with fallback
    ...
    if (body) {
      body.innerHTML = `
        <div class="ddp-error-box">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
          <p>${message}</p>
          <small>Make sure your backend is running on localhost:8000</small>
        </div>
      `;
    }
  }
